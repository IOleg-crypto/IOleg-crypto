<h1 align="center">👋 Hi, I'm Oleg</h1>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=IOleg-crypto&label=Profile%20views&color=0e75b6&style=flat" alt="Profile views" />
</p>

<p align="center">
  🎓 <b>Software Engineering Student</b> @ <i>Lviv Polytechnic National University</i><br>
  💻 Focused on <b>Systems Programming</b>, <b>Computer Graphics</b>, and <b>Desktop Applications</b>.
</p>

<p align="center">
  <a href="https://github.com/IOleg-crypto" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" alt="GitHub">
  </a>
</p>

---

### 🚀 About Me
- 🎓 Software Engineering degree candidate at **Lviv Polytechnic National University**.
- ⚙️ Passionate about **low-level systems development**, custom engine architecture, and graphics pipelines.
- ⚡ Experienced in **C++**, **C#**, multi-threaded architectures, and network application protocols.
- 🎯 Constantly tackling high-performance engineering challenges and exploring modern rendering techniques.

---

### 🎯 Engineering Focus & Architecture
- **Systems & Memory Management:** RAII, smart pointers, custom memory allocators, cache-friendly data structures (ECS).
- **Graphics Pipeline:** OpenGL, GLSL shaders, framebuffer management, font atlas packing (FreeType), ImGui integration.
- **Networking & Concurrency:** Sockets, multi-threaded client-server applications, real-time message passing.
- **Build Tooling:** Multi-config CMake setups, cross-platform compilation, and Git workflows.

---

### 🛠 Tech Stack

**Languages & Core**
<div style="display: flex; gap: 8px; flex-wrap: wrap;">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" height="36" alt="C++" title="C++">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="36" alt="C#" title="C#">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="36" alt="Python" title="Python">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="36" alt="Java" title="Java">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="36" alt="HTML5" title="HTML5">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="36" alt="CSS3" title="CSS3">
</div>

<br>

**Tools, Environment & Frameworks**
<div style="display: flex; gap: 8px; flex-wrap: wrap;">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="36" alt="Git" title="Git">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cmake/cmake-original.svg" height="36" alt="CMake" title="CMake">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/visualstudio/visualstudio-original.svg" height="36" alt="Visual Studio" title="Visual Studio">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" height="36" alt="VSCode" title="VSCode">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jetbrains/jetbrains-original.svg" height="36" alt="JetBrains" title="JetBrains Tools">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" height="36" alt="Figma" title="Figma">
</div>

---

### 📌 Featured Projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h3 align="center">🎮 Custom Game Engine & Render Pipeline</h3>
      <p align="center">
        <img src="https://img.shields.io/badge/C%2B%2B-17%2F20-blue" alt="C++" />
        <img src="https://img.shields.io/badge/OpenGL-GLSL-green" alt="OpenGL" />
        <img src="https://img.shields.io/badge/CMake-Build-orange" alt="CMake" />
      </p>
      <p>Low-level rendering engine architecture featuring shadow mapping, framebuffer management, font atlas packing via FreeType, EnTT (ECS) integration, Jolt Physics, and custom UI powered by ImGui.</p>
      <p align="center">
        <a href="https://github.com/IOleg-crypto"><b>View Repository »</b></a>
      </p>
    </td>
    <td width="50%" valign="top">
      <h3 align="center">💬 Multi-threaded Network Client-Server</h3>
      <p align="center">
        <img src="https://img.shields.io/badge/Java-JavaFX-red" alt="JavaFX" />
        <img src="https://img.shields.io/badge/Sockets-Networking-yellow" alt="Sockets" />
      </p>
      <p>Multi-threaded real-time messaging application. Built with optimized socket handling, asynchronous data processing, and a clean custom desktop user interface.</p>
      <p align="center">
        <a href="https://github.com/IOleg-crypto"><b>View Repository »</b></a>
      </p>
    </td>
  </tr>
</table>

### 📫 Connect with me
<p align="left">
  <a href="https://github.com/IOleg-crypto" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
</p>
